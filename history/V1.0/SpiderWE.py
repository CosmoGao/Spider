...
# 正则匹配
pattern = re.compile(
    r'"loanId":(?P<loanid>\d+).+"amount":(?P<amount>[^,]+),"interest":(?P<interest>[0-9.]+),"months":(?P<months>\d+).*"finishedRatio":(?P<finish_ratio>[^,]+).*"nickName":"(?P<user_name>[^"]+).+"borrowerLevel":"(?P<risk_level>[^"]+)","address":"(?P<address>[^"]*)","jobType":"(?P<job_type>[^"]*)"',
    re.S | re.M)
    ...
    for page_index in range(page_start, page_end):
        ...
        if x != None and len(x) != 0:
            ...
            cursor.execute(
                ...
                (x[0]['loanid'], x[0]['user_name'].decode("utf8"), x[0]['address'].decode("utf8"),
                 x[0]['job_type'].decode("utf8"), x[0]['amount'], x[0]['interest'], x[0]['months'], x[0]['risk_level'],
                 x[0]['finish_ratio']))
            if index == 50:
                index = 0
                WE_db.commit()
